...
\begin{adjustwidth}{0.2in}{0.2in}
      ...
      navigation. The low-level layer employs a cascaded PID control architecture, comparing optimal LQR synthesis with robust $H_\infty$ synthesis (HARE) for attitude and position tracking under external disturbances. The framework is validated through high-fidelity simulation
...
cascaded PID-LQR / PID-$H_\infty$ robust control, validated through high-fidelity
...
cascaded PID control architecture. Both PID-LQR and PID-$H_\infty$ share the
identical cascaded topology, consisting of an outer position control loop
($x$, $y$, $z$) generating reference attitude angles and an inner attitude
control loop ($\phi$, $\theta$, $\dot{\psi}$) regulating quadrotor torques.
The distinction lies in gain synthesis: PID-LQR solves the standard continuous
Algebraic Riccati Equation (ARE), whereas PID-$H_\infty$ solves the $H_\infty$
Riccati Equation (HARE) incorporating an explicit $H_\infty$ disturbance
attenuation bound $\gamma$. Control outputs are converted to motor PWM signals
...
\begin{figure}[ht]
      ...
      \begin{tikzpicture}[
            ...
            {Cascaded Outer Loop\\Position Control $(x,y,z)$\\(PID Gains: LQR / $H_\infty$)};

            \node[box,draw=low,below=0.35cm of pidhinf] (pidlqr)
            {Cascaded Inner Loop\\Attitude Control $(\phi,\theta,\dot{\psi})$\\(PID Gains: LQR / $H_\infty$)};
...
\begin{enumerate}[leftmargin=*,labelsep=0.5em,itemsep=0pt,parsep=0pt,topsep=2pt,partopsep=0pt]
      \item {\fontsize{9}{11}\selectfont Cortes, J.\ et al., \emph{IEEE Trans.\ Robot.\ Autom.} \textbf{20}(2), pp.\ 243--255 (2004).}
      \item {\fontsize{9}{11}\selectfont van den Berg, J.\ et al., in \emph{Robotics Research}, Springer, pp.\ 3--19 (2011).}
      \item {\fontsize{9}{11}\selectfont Raffo, G.V.\ et al., \emph{J.\ Intell.\ Robot.\ Syst.} \textbf{57}(1), pp.\ 303--330 (2010).}
      \item {\fontsize{9}{11}\selectfont Choset, H., \emph{Ann.\ Math.\ Artif.\ Intell.} \textbf{31}(1), pp.\ 113--126 (2001).}
\end{enumerate}
