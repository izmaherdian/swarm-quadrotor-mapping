...
Partitioning a territory among several agents converts a sequential survey into a parallel
one, and the saving grows with the size of the area.

The demands this places on the system are easiest to see in a concrete case. Consider
mapping a landslide scar a few hundred metres across, a few days after the event. The
boundary is irregular and encloses ground that must not be overflown, so the survey region
is non-convex. Debris and standing structures are not on any map, so obstacles must be
found in flight instead of looked up. Gusts channel through the terrain, and a quadrotor
that drifts off its sweep line leaves a gap in the map, not merely a tracking error. If one
vehicle is lost, the area it had been assigned is still unsurveyed, and the remaining
vehicles have to absorb it before the light goes. A system that handles only the first of
these is of little use in the field.

Meeting all four demands at once therefore requires region partitioning that tolerates
non-convex geometry, obstacle handling that does not presume prior knowledge, flight
control that holds a line under disturbance, and coverage that survives the loss of an
agent.
...
carried out on kinematic point-mass models, not on full quadrotor dynamics
...
\begin{enumerate}[nosep,leftmargin=*,topsep=4pt,label=(\roman*)]
    \item A polygon-aware coverage planner whose extremal sweep rows follow the cell
          boundary. This removes the unswept wedges that a fixed-pitch sweep leaves at
          tapered corners.
    \item A CBF-QP that folds static obstacles, moving obstacles, arena walls, and
          reciprocal inter-agent spacing into a single quadratic program. Its
          class-$\mathcal{K}$ function is derived from the identified closed-loop plant, so
          every admissible command is one the vehicle can execute.
    \item A like-for-like comparison of LQR and $H_\infty$ gain synthesis on an identical
          cascade, including an explicit procedure for selecting the attenuation bound
          $\gamma$.
    \item An evaluation on full six-degree-of-freedom dynamics over three non-convex
          regions, under nominal, turbulent, and agent-failure conditions.
\end{enumerate}

The remainder of the paper surveys the related literature, describes the three layers and
the simulation environment, and reports the measured results.
...
kinematic agents, so the turn cost is a count and carries no dynamic penalty.
...
dictated by braking capability, not by a tuning constant. The barrier used here,
...
The stack is formulated per agent. Each vehicle carries its own state machine, its own
sweep assignment, and its own two-variable quadratic program, and the constraint rows of
that program need nothing beyond the agent's own state, the positions and manoeuvring
weights of its neighbours, and the confirmed obstacle set. Nothing in the formulation
requires a global optimisation. The present study nevertheless executes all seven agents
inside a single coordinator process, because the reciprocal split of Eq.~\eqref{eq:lambda}
is exact only when both weights are read from the same instant; a deployment that
distributes the agents across vehicles would have to work with neighbour weights that
arrive over a link and may be stale, which is taken up among the limitations.
...
% Sumbernya Diagram-Swarm-Quadrotor-Mapping.drawio (1 Sep 2026), sudah selaras
% dengan naskah: CBF-QP Solver, "Reciprocal Collision Avoidance", Gazebo tanpa
% PX4 SITL, dan keluaran misi tanpa angka coverage yang dipakukan.
% Kalau .drawio diubah lagi: ekspor JPG -> timpa docs/…-v1.jpg -> salin ke sini.
\begin{figure}[tbp]
    \centering
    \includegraphics[width=\textwidth]{figures/architecture.jpg}
    \caption{Three-layer control architecture for resilient swarm geodetic mapping. The
        per-agent stacks show how the method is formulated: every vehicle carries the full
        stack and needs only its neighbours' states, which is what would allow the agents to
        be distributed. In the study reported here the seven stacks are executed inside one
        coordinator process, so that the reciprocal responsibility split is computed from a
        single swarm snapshot. Command signals flow downward, state feedback upward, and
        heartbeat loss triggers re-partitioning in the FT-CC module.}
    \label{fig:arch}
\end{figure}
...
run for $12$ iterations. Clipping acts on the polygon itself and not on a bounding box,
...
further than $0.70$~m from its anchor is discarded as a moving object and never promoted.
...
class-$\mathcal{K}$ function that is derived from the vehicle, not tuned.
...
same theorem, not from a tuned constant. Speed and rate limits are inscribed regular
...
failure. In this study all agents are solved from a single swarm snapshot, which is what makes the
reciprocal split exact; the rows themselves are per agent, so the same formulation admits a
distributed implementation once stale neighbour weights are accounted for.
...
communication or negotiation. The weights encode who gives way. An agent sweeping a row or holding its
start yaw takes $w=0.25$, so it is protected and its mapped line stays straight. An agent
pausing at a row end takes $w=0.50$, and one in transit, stepping between rows, or
returning to its centroid takes $w=1.00$. An agent that has finished, or is waiting for the
...
$P(\mathrm{tier}>0)$; a value near zero means the ladder is a safety net, not an
...
Figure~\ref{fig:fbd} is the free-body diagram and fixes the conventions used throughout.

% ---------------------------------------------------------------------
% Free-body diagram quadrotor.
% KONVENSI: ENU (z ke ATAS), sesuai Eq. m*pddot = [0,0,-mg] + R[0,0,T].
% JANGAN diganti NED tanpa mengubah persamaan itu juga.
% Drag TIDAK digambar: model di naskah ini memang tidak memuatnya.
% Tata letak rotor diturunkan dari matriks mixer (lihat komentar Eq. mixer).
% Proyeksi isometrik manual: sx = 0.866(X-Y), sy = 0.5(X+Y) + Z.
% ---------------------------------------------------------------------
% SATU gambar free-body diagram 6-DOF. Menggantikan dua gambar terpisah.
% KONVENSI: ENU (z ATAS), sesuai m*pddot = [0,0,-mg] + R[0,0,T].
%           Drag TIDAK digambar — model di naskah ini tidak memuatnya.
% Tata letak rotor diturunkan dari matriks mixer pid_lqr_node.py:
%   tau_x = kf*d(-w1+w2+w3-w4) -> y = (-d,+d,+d,-d)
%   tau_y = kf*d(-w1+w2-w3+w4) -> x = (-d,+d,-d,+d)
%   tau_z = km(-w1-w2+w3+w4)   -> pasangan diagonal {1,2} vs {3,4}
% Proyeksi isometrik: sx = 0.866(X-Y), sy = 0.5(X+Y) + Z.
% Panjang lengan l diuraikan jadi d di x_B dan d di -y_B (garis putus-putus),
% itulah cara menampilkan d = l sin45 tanpa panel tampak-atas terpisah.
% ---------------------------------------------------------------------
\begin{figure}[tbp]
    \centering
    \begin{tikzpicture}[
        every node/.style={font=\scriptsize},
        lbl/.style={font=\tiny},
        arm/.style={line width=2.1pt, draw=black!60, line cap=round},
        rot/.style={circle, draw=black!72, line width=0.65pt, fill=black!7,
                minimum size=0.60cm, inner sep=0pt},
        blade/.style={draw=black!55, line width=0.5pt, fill=black!4},
        thr/.style={-{Latex[scale=0.9]}, draw=layersim, line width=1.25pt},
        tq/.style={-{Latex[scale=0.62]}, draw=layermid, line width=0.75pt},
        eul/.style={-{Latex[scale=0.72]}, draw=layerhigh, line width=0.9pt},
        bax/.style={-{Latex[scale=0.8]}, draw=black!85, line width=0.8pt},
        wax/.style={-{Latex[scale=0.8]}, draw=black!50, line width=0.75pt},
        dimn/.style={{Latex[scale=0.5]}-{Latex[scale=0.5]}, draw=black!65, line width=0.4pt},
        gline/.style={draw=black!40, line width=0.35pt, dashed}
        ]

        % ================= kerangka inersia {W} =================
        \begin{scope}[shift={(2.15,0.70)}]
            \fill[black!8, draw=black!40, line width=0.4pt]
            (0,0) -- (1.30,0.75) -- (0.34,1.30) -- (-0.96,0.55) -- cycle;
            \fill[black!80] (0,0) circle (0.055);
            \draw[wax] (0,0) -- (1.58,0.91) node[lbl, right, inner sep=1.5pt] {$x_W$};
            \draw[wax] (0,0) -- (-1.42,0.82) node[lbl, left, inner sep=1.5pt] {$y_W$};
            \draw[wax] (0,0) -- (0,1.78) node[lbl, above, inner sep=1.5pt] {$z_W$};
            \node[lbl, anchor=north, align=center] at (0.05,-0.22)
            {inertial frame $\{W\}$\\east--north--up};
        \end{scope}

        % gravitasi
        \draw[-{Latex[scale=0.9]}, black!85, line width=1.1pt] (0.10,3.05) -- (0.10,2.00);
        \node[lbl, anchor=south, align=center] at (0.10,3.12) {gravity\\$-z_W$};

        % rotasi badan -> dunia
        \draw[-{Latex[scale=0.85]}, black!70, line width=0.75pt, dashed]
        (3.95,2.05) to[bend left=20] (6.30,2.72);
        \node[lbl, anchor=south] at (5.15,2.96) {$R^W_B(\phi,\theta,\psi)$};

        % ================= wahana =================
        \begin{scope}[shift={(9.05,2.05)}, rotate=-12, scale=1.08]
            \coordinate (c)  at (0,0);
            \coordinate (p1) at (0,-1.15);       % body (-d,-d)
            \coordinate (p2) at (0, 1.15);       % body (+d,+d)
            \coordinate (p3) at (-1.99,0);       % body (-d,+d)
            \coordinate (p4) at ( 1.99,0);       % body (+d,-d)

            % uraian lengan: komponen d sepanjang x_B (komponen -y_B sama besar)
            \draw[gline] (c) -- (0.995,0.575) -- (p4);
            \draw[dimn] (0.34,-0.30) -- (1.26,0.23);
            % Label ditempatkan EKSPLISIT di bawah garis dimensi. Anchor
            % above/below bawaan menaruhnya di y~0.06, tepat di atas lengan
            % rotor 3-4 yang merupakan garis horizontal y=0.
            \node[lbl, inner sep=1pt] at (0.98,-0.2) {$d$};

            \draw[arm] (p1) -- (p2);
            \draw[arm] (p3) -- (p4);
            \fill[black!16, draw=black!60, line width=0.6pt] (c) circle (0.27);

            \foreach \p/\n in {p1/1, p2/2, p3/3, p4/4}{
                    \draw[blade] (\p) ellipse (0.46 and 0.19);
                    \draw[thr] (\p) -- ++(0,1.20);
                    \node[rot] at (\p) {\fontsize{5.4}{6}\selectfont\bfseries \n};}

            % torsi reaksi pada cakram rotor: diagonal {1,2} vs {3,4} berlawanan
            \foreach \p in {p1,p2}
            \draw[tq] ([shift={(18:0.44 and 0.185)}]\p) arc (18:-262:0.44 and 0.185);
            \foreach \p in {p3,p4}
            \draw[tq] ([shift={(162:0.44 and 0.185)}]\p) arc (162:442:0.44 and 0.185);

            % panjang lengan
            \draw[dimn] (-0.20,-0.24) -- node[lbl, below, inner sep=1.5pt] {$l$} (-1.90,-0.24);

            % sumbu badan
            \draw[bax] (c) -- (1.88,1.086) node[lbl, above right, inner sep=1pt] {$x_B$};
            \draw[bax] (c) -- (-1.88,1.086) node[lbl, above left, inner sep=1pt] {$y_B$};
            \draw[bax] (c) -- (0,1.92) node[lbl, left, inner sep=2pt] {$z_B$};

            % sudut Euler di ujung tiap sumbu
            % busur di TENGAH sumbu, label sudut di sisi luar busur
            \begin{scope}[shift={(1.34,0.774)}, rotate=120]
                \draw[eul] (0.23,0) arc (0:282:0.23 and 0.095);
            \end{scope}
            \node[lbl, text=layerhigh] at (1.62,0.44) {$\phi$};
            \begin{scope}[shift={(-1.34,0.774)}, rotate=60]
                \draw[eul] (0.23,0) arc (0:282:0.23 and 0.095);
            \end{scope}
            \node[lbl, text=layerhigh] at (-1.62,0.44) {$\theta$};
            \draw[eul] (0.29,2.16) arc (0:292:0.29 and 0.145);
            \node[lbl, text=layerhigh] at (0.52,2.30) {$\psi$};

            % ditaruh di ruang kosong bawah-kiri, bukan di kanan: sisi kanan
            % sudah dipakai kotak anotasi.
            \node[lbl, text=layersim, anchor=west] at (-2.95,-1.28) {$T_i=k_f\omega_i^2$};
            \node[lbl, text=layermid, anchor=west] at (-2.95,-1.70) {$\tau_i=k_m\omega_i^2$};
        \end{scope}

        % pusat massa dan berat
        \fill[black!85] (9.05,2.05) circle (0.055);
        \draw[gline] (9.05,2.05) -- (10.72,2.05);
        \draw[-{Latex[scale=0.9]}, black!85, line width=1.0pt] (10.72,2.05) -- (10.72,0.62);
        \node[lbl, anchor=west] at (10.80,0.76) {$mg$};

        % ================= blok anotasi =================
        \begin{scope}[shift={(12.30,0.42)}]
            \draw[black!35, line width=0.4pt, rounded corners=2pt] (-0.22,-0.18) rectangle (3.34,3.02);
            \node[lbl, anchor=north west, align=left] at (-0.05,2.90)
            {\textbf{Six degrees of freedom}\\[1pt]
                position $\mathbf{p}=(x,y,z)$ in $\{W\}$\\
                attitude $(\phi,\theta,\psi)$ of $\{B\}$};
            \draw[black!25, line width=0.35pt] (-0.05,1.94) -- (3.17,1.94);
            \node[lbl, anchor=north west, align=left] at (-0.05,1.86)
            {\textbf{Moments from the mixer}\\[1pt]
                $\tau_x>0:\ \omega_2,\omega_3$ raised\\
                $\tau_y>0:\ \omega_2,\omega_4$ raised\\
                $\tau_z>0:\ \omega_3,\omega_4$ raised};
            \draw[black!25, line width=0.35pt] (-0.05,0.66) -- (3.17,0.66);
            \node[lbl, anchor=north west, align=left] at (-0.05,0.58)
            {$T\cos\theta=mg\ \Rightarrow\ a=g\tan\theta$\\
                $a_{\max}=g\tan\theta_{\max}$};
        \end{scope}

    \end{tikzpicture}
    \caption{Free-body diagram of the quadrotor and the conventions used throughout. The
        inertial frame $\{W\}$ is east--north--up, so gravity acts along $-z_W$ and the
        translational dynamics take the form of Eq.~\eqref{eq:e24}; $R^W_B(\phi,\theta,\psi)$
        maps body to world; the weight acts at the centre of mass, marked by the dot. Roll $\phi$ is about $x_B$, pitch $\theta$ about $y_B$ and yaw
        $\psi$ about $z_B$ shown in their positive senses. Rotor $i$ produces thrust
        $T_i=k_f\omega_i^2$ along $+z_B$ and a reaction torque $\tau_i=k_m\omega_i^2$ opposing
        its own rotation; the diagonal pairs $\{1,2\}$ and $\{3,4\}$ counter-rotate, so the
        reaction torques cancel in hover and net yaw comes from unbalancing them. The dashed
        construction resolves the arm $l$ into its components $d=l\sin45^\circ$ along $x_B$
        and $-y_B$, the moment arm over which differential thrust produces roll and pitch.
        Because altitude is held, $T\cos\theta=mg$ and the tilt limit caps lateral
        acceleration at $a_{\max}=g\tan\theta_{\max}$ --- the bound on which the barrier of
        Eq.~\eqref{eq:e14} is built, which is what ties the safety layer to the vehicle rather
        than to a tuning constant. Aerodynamic drag is not shown because it is not modelled
        here; its omission is revisited among the limitations.}
    \label{fig:fbd}
\end{figure}


% ---------------------------------------------------------------------
% Skematik quadrotor + gaya + konvensi roll/pitch/yaw.
% Tata letak rotor DITURUNKAN dari matriks mixer (pid_lqr_node.py):
%   tau_x = kf*d(-w1+w2+w3-w4)  -> y = (-d,+d,+d,-d)
%   tau_y = kf*d(-w1+w2-w3+w4)  -> x = (-d,+d,-d,+d)
%   tau_z = km(-w1-w2+w3+w4)    -> pasangan diagonal {1,2} vs {3,4}
% Proyeksi isometrik manual: sx=(X-Y)cos30, sy=(X+Y)sin30+Z.
...
so that the closed loop satisfies $\|T_{zw}\|_\infty<\gamma$. Both syntheses produce a state-feedback gain $K_a$. It is mapped onto the PID triple
...
attributable to the stressor just introduced, not to an untangleable combination.
...
attributable to agent loss, not to the obstacle field. Every mission logs per-agent
...
nominal tier almost everywhere, so the relaxation ladder is a safety net, not an
operating mode.
...
\emph{Execution is centralised.} The formulation is per agent, but the seven agents are
solved inside one process so that the reciprocal split of Eq.~\eqref{eq:lambda} is exact.
Distributing them across vehicles would replace each neighbour's current weight $w_j$ with
a value received over a link. Because $w_j$ changes whenever agent $j$ changes mission
state, a stale value breaks $\lambda_{ij}+\lambda_{ji}=1$ and with it the joint condition
$\dot h\ge-\varphi(h)$: if both agents happen to believe they are the protected one, neither
gives way. A distributed deployment would therefore need either a conservative fallback,
$\lambda_{ij}=1$ whenever the last update from $j$ is older than a bounded age, or a fixed
symmetric split at the cost of the mission-state weighting. Neither variant is evaluated
here. The partitioning layer is less affected, since decentralised centroidal Voronoi
coverage is established \cit{cortesCoverageControl2004}, whereas FT-CC would additionally
require the survivors to agree on which agent has failed and which rows each will take.
...
identified closed-loop plant, not tuned by hand. The low-level layer compares LQR and
