...
CBF-QP safety filter whose class-$\mathcal{K}$ function is derived from the closed-loop model of
the vehicle; (ii) a side-by-side comparison of PID gains synthesised
...
occurred. Every reported result is computed by an analysis script from the recorded telemetry.
...
\caption{(a) Gazebo view of the rect scenario (screenshot); (b)--(e) RViz coverage maps (green)
at the end of the \Hinf{} missions in rect, L-shape, U-shape and plus, with lanes, flown paths and
mapped obstacles, extracted from the mission recordings.}
\label{fig:env}
\end{figure}
...
for the \TimeRedMin--\TimeRedMax\% shorter missions; the 95th-percentile tilt was essentially equal,
and so was altitude error outside the two missions with upsets. The linear analysis (Table~\ref{tab:gains}) agrees in direction: the $H_\infty$
...
0.5~m/s$^2$ wind allowance that does not scale with the measured gusts (peaks of
\WindMaxH~m/s). A further \UpRectLqrSecondTilt$^\circ$ upset (\LQR{}, rect, agent 6 at
\UpRectLqrSecondT~s) occurred before sweeping began, with the nearest cylinder
\UpRectLqrSecondSurf~m away, and was recovered. In all \NRunsHdynNeg{} missions some tier-2 solves bound by
a moving-obstacle row also reported a negative minimum barrier value (down to
$\HdynMinLogged$~m), but the log keeps only the minimum over all rows and the moving cylinders
were not logged, so their physical distance cannot be recovered. A rotor-tip collision envelope,
obstacle-aware lane ends, a gust-dependent acceleration budget and logging of the moving
obstacles are the next steps.
...
held the lanes \LatRedMin--\LatRedMax\% more tightly and, where both finished, shortened
missions by \TimeRedMin--\TimeRedMax\% compared with LQR gains, at several times the torque effort and with
smaller margins; both designs grazed static obstacles under turbulence, and one contact ended an LQR
