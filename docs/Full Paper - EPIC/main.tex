...
rejecting the mean wind, and finished \TimeRedMin--\TimeRedMax\% sooner than \LQR{} where both
finished, at
...
\cite{abichandaniMultiQuadrotorWind2023}; LQR and $H_\infty$ synthesis remain common for multirotor
control under wind \cite{doyleStateSpaceHinf1989,zyadatQuadplaneWind2023}, and swarm fault
...
sweeps and lane-level fault reallocation above a LiDAR-mapped CBF-QP whose class-$\mathcal{K}$
function is derived from the closed-loop model of the vehicle; (ii) a comparison of PID gains
from the LQR and $H_\infty$ Riccati equations, on the linear model and inside the swarm; and
(iii) a data-traceable evaluation of eight missions under combined hazards, including their
failures. Every reported result is computed by an analysis script from the telemetry.
...
\item \emph{Moving obstacles.} Two kinematic cylinders ($r=0.45$~m) oscillate along the region
      ...
      i.e.\ peak speeds of 1.6--2.5~m/s.
...
tilt, 20~s start-wait timeout). Metrics use the 20~Hz controller logs without the first 20~s and
...
inter-agent distance; tier: solves needing tier 1--2. Killed: rect \VictimsRectHinf{} (H$_\infty$),
\VictimsRectLqr{} (LQR); L \VictimsLshapeHinf; U \VictimsUshapeHinf; plus \VictimsPlusHinf.}
\label{tab:results}
\footnotesize
...
\LQR{} U-shape mission ended at \CovUshapeLqr\% with thin uncovered strips along cell boundaries in
the left arm, nearly absent for \Hinf{}; their cause was not isolated. The \LQR{} rect mission
was aborted at \TmisRectLqr~s.

\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figures/fig_coverage.pdf}
\caption{Coverage against mission time (coordinator clock). Triangles: agent kills, after which the failed cell is re-swept.}
\label{fig:coverage}
\end{figure*}
...
\LatRmsLqrMin--\LatRmsLqrMax~cm to \LatRmsHinfMin--\LatRmsHinfMax~cm (\LatRedMin--\LatRedMax\%) in
all four regions. Most of the difference is a mean offset: \LQR{} flew $\LatMeanLqrMax$ to
...
and the \LQR{} node freezes its position integrator beyond 0.25~m of error, which happened in
\IntegOffLqrMin--\IntegOffLqrMax\% of its sweep samples. Mean sweep speeds of
\VHinfMin--\VHinfMax{} against \VLqrMin--\VLqrMax~m/s (command 1.6~m/s) account for the
...
of \LQR{} (Fig.~\ref{fig:tracking}b), with motor saturation below 0.2\% of samples, and its outer
...
\ObssurfLshapeLqr{} and \ObssurfUshapeLqr~m under \LQR{} in rect, L- and U-shape; in \emph{plus}
both passed within \ObssurfPlusHinf~m of the central cylinder, inside the rotor envelope, where
the assumed guard takes the contact. The upsets (Fig.~\ref{fig:events}) share a pattern: each
...
\UpRectLqrTilt$^\circ$ and fell. In \emph{plus}, agent 2 of both missions grazed the central
...
attributed to the controller; they are better explained by obstacle-blind lanes ending inside a
...
reported a negative minimum barrier value (down to $\HdynMinLogged$~m): a margin was entered, but
the log keeps only the minimum over all rows and no moving-cylinder positions, so a contact can be
neither confirmed nor excluded; under the guard assumption it would load the guard. Since the
guard enlarges the envelope, a guard-sized barrier radius, obstacle-aware lane ends, a
gust-dependent acceleration budget and logging of the moving obstacles are the next steps.
...
The results rest on one run per configuration (fixed wind seed), perfect state feedback and
moving-obstacle tracking, instantaneous failure detection, geometric coverage, assumed but
unsimulated rotor guards and centralised upper layers; no ablation or re-implemented baseline.
...
turbulence, unmapped and moving obstacles, and the loss of two of seven quadrotors. $H_\infty$
gains held the lanes \LatRedMin--\LatRedMax\% more tightly and, where both finished, shortened
missions by \TimeRedMin--\TimeRedMax\% relative to LQR gains, at several times the torque effort and
with smaller margins; both designs grazed static obstacles, and one contact ended an LQR mission.
Next steps are repeated runs over wind seeds, a guard-sized, wind-aware barrier, distributed upper
