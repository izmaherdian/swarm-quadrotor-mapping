...
with Lloyd's iteration distributes agents over a region
\cite{lloydLeastSquaresQuantization1982,duCentroidalVoronoiTessellations1999,cortesCoverageControl2004}
and has been extended to non-convex domains
\cite{breitenmoserVoronoiCoverage2010,caoDistributedWeightedCoverage2023} and to agent dropout
\cite{leeAdaptiveCentroidalVoronoi2024}; boustrophedon sweeps are the standard way to cover a
cell \cite{chosetBoustrophedonDecomposition1998,chosetCoverageSurvey2001,galceranCoveragePathPlanning2013,huRoutePlanning2021},
...
\cite{amesControlBarrierFunction2017,amesControlBarrierFunctions2019}, including reciprocal
multi-robot forms \cite{wangSafetyBarrierCertificates2017,zhouBufferedVoronoiCells2017}; LQR and
$H_\infty$ synthesis are classical routes to quadrotor control
\cite{belkheiriLinearizationControl2012,doyleStateSpaceHinf1989}, and fault tolerance in swarms
is an active topic \cite{okeeffeAnticipatingDegradation2025}.
...
\cite{cortesCoverageControl2004}, and agents are assigned to cells by the Hungarian algorithm.
...
\item \emph{Wind.} A first-order Gauss--Markov process per horizontal axis ($\sigma=2.5$~m/s,
      correlation time 0.5~s, seed 42), a Dryden-like surrogate
