...
for pid in $(ps aux | grep -E "gz.sim|ros2 launch|parameter_bridge|ai_iris|pid_lqr_node" | grep -v grep | awk '{print $2}'); do
...
echo "=== 2. Launching Gazebo simulation (Headless) ==="
ros2 launch swarm_sim sim_launch.py \
    num_drones:=1 controller:=pid_lqr_node \
    headless:=true rviz:=false \
    results_base:=single_agent > /tmp/sim_test_flight.log 2>&1 &
SIM_PID=$!
...
if [ ! -f "$CSV_PATH" ]; then
    ...
    cat /tmp/sim_test_flight.log | tail -30
...
echo "Monitoring flight until test_mapping finishes all 10 rows..."
...
echo "=== 5. Final Flight Data Analysis ==="
python3 -c "
import csv
with open('$CSV_PATH', 'r') as f:
    rows = list(csv.DictReader(f))
...
if rows:
    # Filter only airborne phase (t >= 5.0s)
    ...
    print(f'Max |tau_y|: {max(abs(ty) for ty in tau_ys):.2f} Nm (Limit: 1.20 Nm)')
    
    # Crash criteria during airborne phase: sustained drop below 1.0m or sustained tilt > 45 deg
