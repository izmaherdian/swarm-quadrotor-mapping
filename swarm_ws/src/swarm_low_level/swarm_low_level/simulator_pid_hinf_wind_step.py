...
def simulate():
    ...
    # 1. Panggil nilai Gain dari Solver H-infinity
    ...
    for k in range(1, n_steps):
        ...
        # GANGGUAN ANGIN (Crosswind -X, -Y, -Z)
        F_wind_x = -2.0 if (k * dt) >= 3.0 else 0.0
        F_wind_y = -2.0 if (k * dt) >= 3.0 else 0.0
        F_wind_z = -2.0 if (k * dt) >= 3.0 else 0.0
    ...
    # 5.5 Perhitungan Metrik Evaluasi Kinerja (RMSE & Eu)
    ...
    # [1] X
    plt.subplot(4, 3, 1)
    plt.plot(time, states[0, :], 'b-', linewidth=2, label='Aktual (PID-Hinf)')
    ...
    # [2] Y
    plt.subplot(4, 3, 2)
    plt.plot(time, states[4, :], 'b-', linewidth=2, label='Aktual (PID-Hinf)')
    ...
    # [3] Z
    plt.subplot(4, 3, 3)
    plt.plot(time, states[8, :], 'b-', linewidth=2, label='Aktual (PID-Hinf)')
    ...
    # [4] Roll (phi)
    plt.subplot(4, 3, 4)
    plt.plot(time, np.rad2deg(states[6, :]), 'b-', linewidth=2, label='Aktual (PID-Hinf)')
    ...
    # [5] Pitch (theta)
    plt.subplot(4, 3, 5)
    plt.plot(time, np.rad2deg(states[2, :]), 'b-', linewidth=2, label='Aktual (PID-Hinf)')
    plt.plot(time, np.zeros_like(time), 'r--', linewidth=1.5, label='Referensi')
    plt.xlabel('t [s]'); plt.ylabel('Pitch [deg]'); plt.title('Sudut Pitch (theta)'); plt.grid(True); plt.legend()
    
    # [6] Yaw (psi)
    plt.subplot(4, 3, 6)
    plt.plot(time, np.rad2deg(states[10, :]), 'b-', linewidth=2, label='Aktual (PID-Hinf)')
    ...
    plt.tight_layout()
    plt.suptitle('Evaluasi Kinerja PID-Hinf Synthesis (Wind Step -X, -Y, -Z)', fontsize=16, fontweight='bold')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig('grafik_pid_hinf_wind_step.png', dpi=300)
