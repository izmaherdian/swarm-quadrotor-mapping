...
cleanup() {
    ...
    echo ""
    echo "=== CSV hasil 2 Drone tersimpan di: ==="
    ...
    echo ""
    echo "Done (Double-Agent Selesai)."
}
...
echo "=== Launch Gazebo sim (background) [RViz2: $RVIZ_MODE] ==="
nohup ros2 launch swarm_sim sim_launch.py \
    num_drones:=2 controller:=pid_lqr_node \
    headless:=false rviz:=$RVIZ_MODE \
    results_base:=multi_agent "${EXTRA_ARGS[@]}" > /tmp/sim_double.log 2>&1 &
SIM_PID=$!
echo "  sim PID=$SIM_PID"

echo "=== Wait for ORCA initialized ==="
for i in $(seq 1 30); do
    if grep -q "initialized" /tmp/sim_double.log 2>/dev/null; then
...
if ! grep -q "initialized" /tmp/sim_double.log 2>/dev/null; then
    echo "  ERROR: ORCA tidak pernah initialized. Cek /tmp/sim_double.log"
    exit 1
fi

echo "============================================"
echo "  SIM READY — 2 Drones (iris_1 & iris_2) spawned"
echo "  Jalankan perintah ini di terminal terpisah:"
echo ""
echo "    cd $WS_DIR"
echo "    python3 test_trajectory_2.py"
echo ""
