...
for node in collision_avoidance_node pid_lqr_node tf_prefix_node swarm_mapping_coordinator; do
    chmod +x "$WS_DIR/install/swarm_high_level/lib/swarm_high_level/$node" 2>/dev/null || true
    ...
    sed -i '1s|^#!".*|#!/usr/bin/env python3|' "$WS_DIR/install/swarm_high_level/lib/swarm_high_level/$node" 2>/dev/null || true
...
cleanup() {
    ...
    echo ""
    echo "Done (Single-Agent 2 Selesai)."
}
trap cleanup EXIT INT TERM

echo "=== Launch Gazebo sim + RViz2 (background) ==="
nohup ros2 launch swarm_sim sim_launch.py \
    num_drones:=1 controller:=pid_lqr_node \
    headless:=false rviz:=true \
    results_base:=single_agent > /tmp/sim_single_2.log 2>&1 &
SIM_PID=$!
echo "  sim PID=$SIM_PID"

echo "=== Wait for ORCA initialized ==="
for i in $(seq 1 30); do
    if grep -q "initialized" /tmp/sim_single_2.log 2>/dev/null; then
...
if ! grep -q "initialized" /tmp/sim_single_2.log 2>/dev/null; then
    echo "  ERROR: ORCA tidak pernah initialized. Cek /tmp/sim_single_2.log"
    exit 1
fi

echo "========================================================================="
echo "  SIM READY — Single Drone (iris_1) spawned at (-5.5, -5.5) | RViz2 Active"
echo "  Jalankan perintah ini di terminal terpisah:"
echo ""
echo "    cd $WS_DIR"
echo "    python3 test_mapping.py"
echo ""
