...
%% ========================================================================
%  TAHAP 1: Parameter Fisik Wahana & Robustness H-Infinity
% ========================================================================
g  = 9.81; 
m  = 1.0;
Ix = 8.1e-3; 
...
%% ========================================================================
%  TAHAP 2: Sintesis Gain Kontroler PID-Hinf Desentralisasi
% ========================================================================
...
Q_x_out = diag([1 0.1]); 
R_x_out = 100; 
[Kp_x_out, Ki_x_out, Kd_x_out, Kx_x_out, Ku_x_out] = solve_pid_hinf(A_x_out, B_x_out, C_x_out, Q_x_out, R_x_out, gamma_out);
...
Q_x_in = diag([10 1]);
R_x_in = 0.01; 
[Kp_x_in, Ki_x_in, Kd_x_in, Kx_x_in, Ku_x_in] = solve_pid_hinf(A_x_in, B_x_in, C_x_in, Q_x_in, R_x_in, gamma_in);
...
Q_y_out = diag([1 0.1]); 
R_y_out = 100; 
[Kp_y_out, Ki_y_out, Kd_y_out, Kx_y_out, Ku_y_out] = solve_pid_hinf(A_y_out, B_y_out, C_y_out, Q_y_out, R_y_out, gamma_out);
...
Q_y_in = diag([10 1]);
R_y_in = 0.01; 
[Kp_y_in, Ki_y_in, Kd_y_in, Kx_y_in, Ku_y_in] = solve_pid_hinf(A_y_in, B_y_in, C_y_in, Q_y_in, R_y_in, gamma_in);

% ── 3. SUBSISTEM Z (Heave - Orde 2) ──────────────────────────────────────
Az = [0 1; 0 0];
Bz = [0; 1/m];
Cz = [1 0]; 
[Kp_z, Ki_z, Kd_z, Kx_z, Ku_z] = solve_pid_hinf(Az, Bz, Cz, diag([10 1]), 1, gamma_in);

% ── 4. SUBSISTEM YAW (Orde 2) ────────────────────────────────────────────
Ayaw = [0 1; 0 0];
Byaw = [0; 1/Iz];
Cyaw = [1 0]; 
[Kp_yaw, Ki_yaw, Kd_yaw, Kx_yaw, Ku_yaw] = solve_pid_hinf(Ayaw, Byaw, Cyaw, diag([10 1]), 1, gamma_in);

%% ========================================================================
%  TAHAP 3: Display Hasil Kalkulasi Gain
% ========================================================================
fprintf('=== GAIN PID DECENTRALIZED (OPTIMAL-ROBAS H-INFINITY) ===\n');
...
% ========================================================================
fprintf('[INFO] Menjalankan simulasi PID-Hinf Synthesis di Simulink...\n');
...
out = sim('quadrotor_PID_Hinf_synthesis_decentralized_simulink.slx'); 
...
fprintf('\n=========================================\n');
fprintf(' HASIL EVALUASI METRIK KINERJA PID-HINF\n');
fprintf('=========================================\n');
...
plot_results_pid_hinf(t_g, Xs, Us, Xref, p);
fprintf('[SELESAI] Seluruh kurva pelacakan PID-Hinf Synthesis sukses di-plot.\n');
...
% -------------------------------------------------------------------------
function plot_results_pid_hinf(t_vec, chi_opt, u_opt, Xref, p)
    ...
    figure('Name', 'Gambar 1: Animasi Real-Time Kinerja PID-Hinf Kuadrotor (+ Config)', ...
           'Position', [40, 40, 1450, 850]);

    subplot(3, 4, [1 2 5 6 9 10]);
    hold on; grid on; view(35, 25);
    xlabel('X [m]'); ylabel('Y [m]'); zlabel('Z [m]');
    title('Visualisasi Geometri Animasi Real-Time Mekanika Terbang PID-Hinf Synthesis');
    ...
    sgtitle('Gambar 1: Visualisasi Animasi Real-Time Penerbangan Kuadrotor PID-Hinf Synthesis (+ Config) dan Penjejakan Profil Status');
    ...
    figure('Name', 'Gambar 2: Pelacakan Komparatif Berfilter PID-Hinf Synthesis (3x3 Grid)', ...
           'Position', [50, 50, 1350, 820]);

    subplot(3,3,1); plot(t_vec, chi_opt(:,1), 'b-', 'LineWidth', 2); hold on; plot(t_vec, x_ref_f, 'r--', 'LineWidth', 1.5);
    xlabel('t [s]'); ylabel('x [m]'); title('Pelacakan Posisi X'); legend('Aktual (PID-Hinf)', 'Referensi Filter'); grid on;

    subplot(3,3,2); plot(t_vec, chi_opt(:,3), 'b-', 'LineWidth', 2); hold on; plot(t_vec, y_ref_f, 'r--', 'LineWidth', 1.5);
    xlabel('t [s]'); ylabel('y [m]'); title('Pelacakan Posisi Y'); legend('Aktual (PID-Hinf)', 'Referensi Filter'); grid on;

    subplot(3,3,3); plot(t_vec, chi_opt(:,5), 'b-', 'LineWidth', 2); hold on; plot(t_vec, z_ref_f, 'r--', 'LineWidth', 1.5);
    xlabel('t [s]'); ylabel('z [m]'); title('Pelacakan Ketinggian Z'); legend('Aktual (PID-Hinf)', 'Referensi Filter'); grid on;
    ...
    xlabel('t [s]'); ylabel('\psi [deg]'); title('Sudut Yaw (\psi)'); legend('Aktual (PID-Hinf)', 'Referensi Filter'); grid on;
    ...
    figure('Name', 'Gambar 3: Sinyal Kontrol Optimal PID-Hinf Synthesis u^*(t)', ...
...
% -------------------------------------------------------------------------
function [Kp, Ki, Kd, K_x, K_u] = solve_pid_hinf(A, B, C, Q_val, R_val, gamma)
    n = size(A, 1);
    m = size(B, 2);
    
    % 1. AUGMENTASI SISTEM
    A_aug = [A, B; zeros(m, n), zeros(m, m)];
    Bu_aug = [zeros(n, m); eye(m)]; % Jalur Kontrol Integrator (udot)
    Bw_aug = [B; zeros(m, m)];      % Asumsi Gangguan (Angin/Cross-coupling) memukul Plant
    ...
    R_tilde = [R_val * eye(m), zeros(m, m);
               zeros(m, m),   -gamma^2 * eye(m)];
    
    [P_inf, ~, ~] = care(A_aug, B_tilde, Q_aug, R_tilde);
    
    % Mengekstrak Gain khusus untuk jalur kontrol u (Mengabaikan gain jalur w)
    Ka = (1/R_val) * Bu_aug' * P_inf; 
