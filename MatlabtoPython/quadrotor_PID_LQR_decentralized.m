...
%% ── TAHAP 1: Parameter Fisik Wahana (Nominal) ───────────────────────────
g = 9.81; 
m = 1.0;
Ix = 8.1e-3; 
Iy = 8.1e-3; 
...
%% ── TAHAP 2: Sintesis Gain Kontroler PID-LQR Desentralisasi ─────────────
...
Q_x_out = diag([1 10]); 
R_x_out = 100; 
[Kp_x_out, Ki_x_out, Kd_x_out, Kx_x_out, Ku_x_out] = solve_pid_lqr(A_x_out, B_x_out, C_x_out, Q_x_out, R_x_out);
...
Q_x_in = diag([10 1]);
R_x_in = 0.01; 
[Kp_x_in, Ki_x_in, Kd_x_in, Kx_x_in, Ku_x_in] = solve_pid_lqr(A_x_in, B_x_in, C_x_in, Q_x_in, R_x_in);

% 1.3 MODEL GABUNGAN: X dan Pitch (Orde 4)
A_x_pitch = [0 1  0  0; 
             0 0  g  0;  
             0 0  0  1; 
             0 0  0  0];
B_x_pitch = [0; 0; 0; 1/Iy];
C_x_pitch = [1 0 0 0]; 

A_aug_x = [A_x_pitch, B_x_pitch; zeros(1,4), 0];
B_aug_x = [zeros(4,1); 1];
Q_aug_x = diag([100, 1, 10, 1, 1000]); 
R_aug_x = 1;

K_opt_x = lqr(A_aug_x, B_aug_x, Q_aug_x, R_aug_x); 
Kx_x_pitch = K_opt_x(1:4); 
Ku_x_pitch = K_opt_x(5);   
...
Q_y_out = diag([1 10]); 
R_y_out = 100; 
[Kp_y_out, Ki_y_out, Kd_y_out, Kx_y_out, Ku_y_out] = solve_pid_lqr(A_y_out, B_y_out, C_y_out, Q_y_out, R_y_out);
...
Q_y_in = diag([10 1]);
R_y_in = 0.01; 
[Kp_y_in, Ki_y_in, Kd_y_in, Kx_y_in, Ku_y_in] = solve_pid_lqr(A_y_in, B_y_in, C_y_in, Q_y_in, R_y_in);

% 2.3 MODEL GABUNGAN: Y dan Roll (Orde 4)
A_y_roll = [0 1  0  0; 
            0 0 -g  0; 
            0 0  0  1; 
            0 0  0  0];
...
A_aug = [A_y_roll, B_y_roll; zeros(1,4), 0];
B_aug = [zeros(4,1); 1];
Q_aug = diag([100, 1, 10, 1, 1000]); 
R_aug = 1;

K_opt = lqr(A_aug, B_aug, Q_aug, R_aug); 
Kx_y_roll = K_opt(1:4); 
Ku_y_roll = K_opt(5);   
...
Bz = [0; 1/m];
Cz = [1 0]; 
[Kp_z, Ki_z, Kd_z, Kx_z, Ku_z] = solve_pid_lqr(Az, Bz, Cz, diag([1 10]), 1);

% === 4. SUBSISTEM YAW (Orde 2) ===
Ayaw = [0 1; 0 0];
Byaw = [0; 1/Iz];
Cyaw = [1 0]; 
[Kp_yaw, Ki_yaw, Kd_yaw, Kx_yaw, Ku_yaw] = solve_pid_lqr(Ayaw, Byaw, Cyaw, diag([1 5]), 1);

%% ── TAHAP 3: Eksekusi Simulink & Ekstraksi Struktur Objek 'out' ─────────
fprintf('[INFO] Menjalankan simulasi PID-LQR di Simulink...\n');
...
out = sim('quadrotor_PID_LQR_decentralized_simulink.slx'); 
fprintf('-> Jalur simulasi numerik sukses diselesaikan.\n');

% Ekstraksi variabel dari objek data out Simulink
t_g  = out.tout;   
Xs   = out.Xs;     
Us   = out.Us;     
Xref = out.Xref;   

% Pemetaan data referensi kalang tertutup
x_ref_total   = Xref(:,1);
y_ref_total   = Xref(:,2);
z_ref_total   = Xref(:,3);
psi_ref_total = rad2deg(Xref(:,4)); 

%% ── TAHAP 4: Perhitungan Kuantitatif Metrik Evaluasi Kinerja ────────────
% Perhitungan nilai RMSE individual terhadap filter perintah Simulink
...
rmse_psi_total = sqrt(mean((rad2deg(Xs(:,11)) - psi_ref_total).^2));
...
fprintf('\n=========================================\n');
fprintf('     HASIL EVALUASI METRIK KINERJA PID-LQR\n');
fprintf('=========================================\n');
...
%% ── TAHAP 5: Fungsi Render Visualisasi Grafik Terstruktur ───────────────
fprintf('\n[INFO] Merender visualisasi grafik ke layar...\n');
plot_results_pid_lqr(t_g, Xs, Us, Xref, p);
fprintf('[SELESAI] Seluruh kurva pelacakan PID-LQR sukses di-plot.\n');


% =========================================================================
%  LOCAL FUNCTIONS (Khusus Penanganan Visualisasi dan Solver)
% =========================================================================
...
% -------------------------------------------------------------------------
function [Kp, Ki, Kd, K_x, K_u] = solve_pid_lqr(A, B, C, Q_val, R_val)
    n = size(A, 1);
    m = size(B, 2);
    
    A_aug = [A, B; zeros(m, n), zeros(m, m)];
    B_aug = [zeros(n, m); eye(m)];
    
    Q_aug = blkdiag(Q_val, 0.1 * eye(m));
    R_aug = R_val * eye(m);
    Ka = lqr(A_aug, B_aug, Q_aug, R_aug);
    
    Gamma = [C', (C*A)', (C*(A^2))'; zeros(m, m), (C*B)', (C*A*B)'];
    K_hat = pinv(Gamma) * Ka'; 
    
    k1_hat = K_hat(1:m, :)';
    k2_hat = K_hat(m+1:2*m, :)';
    k3_hat = K_hat(2*m+1:3*m, :)';
...
% -------------------------------------------------------------------------
function plot_results_pid_lqr(t_vec, chi_opt, u_opt, Xref, p)
...
figure('Name', 'Gambar 1: Animasi Real-Time Kinerja PID-LQR Kuadrotor (+ Config)', ...
       'Position', [40, 40, 1450, 850]);

subplot(3, 4, [1 2 5 6 9 10]);
hold on; grid on; view(35, 25);
xlabel('X [m]'); ylabel('Y [m]'); zlabel('Z [m]');
title('Visualisasi Geometri Animasi Real-Time Mekanika Terbang PID-LQR');
...
sgtitle('Gambar 1: Visualisasi Animasi Real-Time Penerbangan Kuadrotor PID-LQR (+ Config) dan Penjejakan Profil Status');
...
figure('Name', 'Gambar 2: Pelacakan Komparatif Berfilter PID-LQR (3x3 Grid)', ...
       'Position', [50, 50, 1350, 820]);

subplot(3,3,1); plot(t_vec, chi_opt(:,1), 'b-', 'LineWidth', 2); hold on; plot(t_vec, x_ref_f, 'r--', 'LineWidth', 1.5);
xlabel('t [s]'); ylabel('x [m]'); title('Pelacakan Posisi X'); legend('Aktual (PID-LQR)', 'Referensi Filter'); grid on;

subplot(3,3,2); plot(t_vec, chi_opt(:,3), 'b-', 'LineWidth', 2); hold on; plot(t_vec, y_ref_f, 'r--', 'LineWidth', 1.5);
xlabel('t [s]'); ylabel('y [m]'); title('Pelacakan Posisi Y'); legend('Aktual (PID-LQR)', 'Referensi Filter'); grid on;

subplot(3,3,3); plot(t_vec, chi_opt(:,5), 'b-', 'LineWidth', 2); hold on; plot(t_vec, z_ref_f, 'r--', 'LineWidth', 1.5);
xlabel('t [s]'); ylabel('z [m]'); title('Pelacakan Ketinggian Z'); legend('Aktual (PID-LQR)', 'Referensi Filter'); grid on;
...
xlabel('t [s]'); ylabel('\psi [deg]'); title('Sudut Yaw (\psi)'); legend('Aktual (PID-LQR)', 'Referensi Filter'); grid on;
...
figure('Name', 'Gambar 3: Sinyal Kontrol Optimal PID-LQR u^*(t)', ...
